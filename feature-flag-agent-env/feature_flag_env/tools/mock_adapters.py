...
class MockDatadogTool(Tool):
    """
    Simulated Datadog integration.

    Actions:
        get_error_rate    — returns error rate from simulator state
        get_latency       — returns latency from simulator state
        get_active_alerts — returns alerts based on thresholds
    """
    ...
    def _execute(self, action_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        if action_name == "get_error_rate":
            return self._mock_error_rate(params)
        elif action_name == "get_latency":
            return self._mock_latency(params)
    ...
    def _mock_error_rate(self, params: Dict) -> Dict:
        # Use actual simulator error rate + small noise
        base_rate = self._env_state.get("error_rate", 0.02)
        noise = random.uniform(-0.005, 0.005)
        return {
            "service": params.get("service_name", "unknown"),
            "latest_value": max(0.0, base_rate + noise),
            "query": "mock:error_rate",
        }

    def _mock_latency(self, params: Dict) -> Dict:
        base_latency = self._env_state.get("latency_p99_ms", 120.0)
        noise = random.uniform(-10, 10)
        return {
            "service": params.get("service_name", "unknown"),
            "latest_value": max(0.0, base_latency + noise),
            "query": "mock:latency_p99",
        }

    def _mock_alerts(self, params: Dict) -> Dict:
        alerts = []
        error_rate = self._env_state.get("error_rate", 0.02)
        latency = self._env_state.get("latency_p99_ms", 120.0)
